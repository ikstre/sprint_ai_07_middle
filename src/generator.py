...
질문에 포함된 핵심 항목명(예: 평가항목, 선정기준, 데이터 이관, 사업기간, 제출기한, 보안요건)은 답변에 그대로 포함하세요.
"""
...
[답변 작성 규칙]
- 문서 근거 기반으로만 답변
- 답변 첫 부분에 반드시 `[핵심 필드]` 섹션을 만들고 확인된 값을 항목별로 먼저 정리
- 비교 질문이면 항목별 표 또는 목록
...
- 근거를 언급할 때는 `근거 청크 1`, `근거 청크 2`처럼 청크 기준으로 표현하고, 발주기관/사업명/파일명 중 확인 가능한 식별 정보를 함께 적기
"""
...
class RAGGenerator:
    ...
    @staticmethod
    def _extract_line_value(text: str, patterns: list[tuple[str, str]]) -> dict[str, str]:
    ...
    def _build_field_candidates(self, retrieved_docs: list[dict], query: str) -> str:
        if not retrieved_docs:
            return "문서에서 확인된 필드 후보 없음"

        patterns = [
            ("발주기관", r"(?:발주기관|발주 기관|수요기관|발주처|주관기관)\s*[:：]?\s*(.+)"),
            ("사업명", r"(?:사업명|과업명|용역명|사업명칭)\s*[:：]?\s*(.+)"),
            ("사업금액", r"(?:사업금액|사업 금액|예산(?:금액)?|추정금액|기초금액)\s*[:：]?\s*(.+)"),
            ("사업기간", r"(?:사업기간|용역기간|수행기간|계약기간|개발기간)\s*[:：]?\s*(.+)"),
            ("제출기한", r"(?:제출기한|제안서 제출기한|제출마감|입찰서 제출마감일시|마감일시)\s*[:：]?\s*(.+)"),
            ("제출방식", r"(?:제출방식|제출방법|입찰방식|제출처)\s*[:：]?\s*(.+)"),
        ]

        candidates: dict[str, str] = {}
        query_keywords = []
        for token in [
            "평가항목", "선정기준", "데이터 이관", "키 식별자", "사업기간",
            "제출기한", "제출방식", "보안요건", "요구사항", "사업추진계획서",
            "관리 요구사항", "응답 시간", "성능 기준",
        ]:
            if token in query:
        ...
        for doc in retrieved_docs[:3]:
            ...
            if meta.get("발주기관") or meta.get("발주 기관"):
                candidates.setdefault("발주기관", meta.get("발주기관") or meta.get("발주 기관"))
            if meta.get("사업명"):
                candidates.setdefault("사업명", meta["사업명"])
            if meta.get("사업금액") or meta.get("사업 금액"):
                candidates.setdefault("사업금액", meta.get("사업금액") or meta.get("사업 금액"))

            text = doc.get("text", "")
            for label, value in self._extract_line_value(text, patterns).items():
                candidates.setdefault(label, value)

        lines = []
        for label in ["발주기관", "사업명", "사업금액", "사업기간", "제출기한", "제출방식"]:
            if label in candidates:
                lines.append(f"- {label}: {candidates[label]}")
        if query_keywords:
            lines.append("- 질문 핵심 항목: " + ", ".join(query_keywords))
    ...
    def generate(
        ...
        field_candidates = self._build_field_candidates(retrieved_docs, query)
